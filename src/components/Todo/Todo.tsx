...
export default function Todo(props: { todo: TypeTodo; Getdata: () => void }) {
  const [isCheck, setIsCheck] = useState<boolean>(props.todo.isCompleted);
  const [updateClick, SetUpdateClick] = useState<boolean>(true); // 수정버튼 클릭시 변경
  const updateInput = useRef<HTMLInputElement>(null); // 수정버튼 클릭시 나오는 Input Ref
  return (
